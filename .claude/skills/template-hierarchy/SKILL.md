...
description: >-
  Editing cookiecutter-rails cookiecutter boilerplate — what to port at
  this hierarchy level (Rails); never push specificity
  to ancestor templates or pull child-only config from reference repos
alwaysApply: false
---

# Template hierarchy (language/tool)

Cookiecutter templates in this family stack **general → specific** (meta → language/tool → project). **This repo** is `cookiecutter-rails` (Rails scope). See also the meta `template-hierarchy` skill at the ancestor repo root.
...
| `circleci-logs-before-local-ci` skill | No | Yes (cookiecutter maintenance) | **Yes** — baked Rails apps |
| `typecheck-rbi-and-ci` skill | No | No | **Yes** — baked Rails apps only |
...
| Language hooks (RuboCop, etc.) | No | Generic only (template `ALL.exclude`) | **Yes** — baked Rails hooks |
...
| Direction | Rule |
|-----------|------|
| **Reference → this template** | Port only what matches **this** level's scope (Rails). |
...
| **This template → descendants** | Push Rails-appropriate config **down**; reserve narrower framework/app bits for child templates. |
...
- If this template is more specific than an ancestor, you **may** add Rails-specific rules here that ancestors must not carry.

## Referenced paths must exist here

When porting config that names directories or tools (`.envrc` `PATH_add`, Makefile paths, Overcommit `include`, etc.):

- **If the path does not exist in this repo**, that is a strong signal it belongs in a **descendant** template, not here.
- Example: reference `.envrc` has `PATH_add script` but this template has no `script/` — add `PATH_add script` only in the child cookiecutter that provides `script/`, not upstream.
- Only add `PATH_add`, ignores, hooks, or docs for paths that are present (or will be created) at **this** tier.

## Examples by tier (adjust for where you are in the chain)
